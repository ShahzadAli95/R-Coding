...
Snake case is a special writing convention commonly used in computer programming. When using snake case, variable and/or function names consisting of multiple words are combined by replacing the spaces between them with underscores for example "Toyota_corolla" or "honda_civic". 
...
Every R user is required to create syntactically valid names in R i.e names that can be understood by the R interpreter. To start off, there is generally alot of freedom when naming objects in R and the general rule observed is that you can name an object literally anything from a combination of letters, numbers and the dot or underline characters except for the defined restrictions. Invalid naming conventionsin R include:

* Names starting with a dot followed by a number for example ".007Bond"
* Naming objects using the reserved words in R. Reserved words include: if, else, repeat, while, function, for, in, next, break, TRUE, FALSE, NULL, Inf, NaN, NA, NA_integer_, NA_real_, NA_complex_, NA_character_, ..., ..1, ..2 etc.

When encoutered with a conflict, the R interpreter may take one or more of the following action(s):
...
marcs_new_object <- 'I love lamborghinis'
marcs_new_object
```
...
```{r, eval=FALSE}
marcs_new_combined_object <- c(marcs_new_object, 'Luckily I own a Gallardo', 'and an Aventador!')
...
By calling mean without any arguemtns we get an error stating that the argument is missing with no defaults set.
...
class() is a funtion that is employed to practise the object oriented style of programming. It creates vectors of values or characters to define datasets that require processing. 

str() is used to display the internal structure of an object much like summary does but in a more compact manner. It does this by only displaying one line for each basic structure.
...
First error was trying to load the gapminder library. I had installed it before in class as well but i dont know why i had to install it again using the install.packages() function.

Another error i faced was while i was installing the gapminder library. I used the install.packages(gapminder) function but forgot to add the quotation marks around gapminder so I again tried with install.packages("gapminder") and it worked.

I was already aware of the knitting problems everybody was facing and with a little research on Discourse (Thanks to Faaez) I knew that the 'Always install missing packages on-the-fly' needs to be ticked from the settings tab. For me, while I was installing MikTex for the frist time, it prompted itself about the option regarding the missing packages, and I chose the correct option. Because of this I solved the pdf knitting error without ever facing it. 

I have yet to face a fourth error to complete this list, perhaps with more coding more errors will pop up with time.
